...
/// Describes a gap between two blocks where a dragged block can be dropped.
///
/// [index] is the zero-based post-removal insertion index — i.e. the index
/// the block would occupy after being removed from its original position and
/// re-inserted at this gap.
///
/// [lineY] is the y-coordinate (in the layout's local coordinate space)
/// where a horizontal insertion indicator line should be drawn.
class BlockInsertionGap {
  /// Creates a [BlockInsertionGap].
  const BlockInsertionGap({required this.index, required this.lineY});

  /// The post-removal insertion index.
  final int index;

  /// The y-coordinate for the insertion indicator line.
  final double lineY;

  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    return other is BlockInsertionGap && other.index == index && other.lineY == lineY;
  }

  @override
  int get hashCode => Object.hash(index, lineY);

  @override
  String toString() => 'BlockInsertionGap(index: $index, lineY: $lineY)';
}
...
class RenderDocumentLayout extends RenderBox
  ...
  // ---------------------------------------------------------------------------
  // Drag-to-move gap computation
  // ---------------------------------------------------------------------------

  /// Returns the [BlockInsertionGap] closest to [localOffset] for a drag
  /// originating from the block identified by [dragNodeId].
  ///
  /// The method walks non-float children, computes midpoints between
  /// consecutive blocks, and finds the gap nearest to [localOffset.dy].
  /// The returned [BlockInsertionGap.index] uses post-removal semantics:
  /// it represents the index the block would occupy after being removed
  /// from its original position and re-inserted.
  ///
  /// Returns `null` when there are fewer than two non-float children, or when
  /// [dragNodeId] does not match any child.
  BlockInsertionGap? getInsertionGapAtOffset(Offset localOffset, String dragNodeId) {
    // Collect non-float children in document order.
    final nonFloats = <RenderDocumentBlock>[];
    RenderDocumentBlock? child = firstChild;
    while (child != null) {
      final pd = child.parentData as DocumentBlockParentData;
      if (!pd.isFloat) {
        nonFloats.add(child);
      }
      child = childAfter(child);
    ...
    if (nonFloats.length < 2) return null;

    // Find the index of the dragged node in the non-float list.
    int dragNodeIndex = -1;
    for (var i = 0; i < nonFloats.length; i++) {
      if (nonFloats[i].nodeId == dragNodeId) {
        dragNodeIndex = i;
        break;
      }
    }
    if (dragNodeIndex == -1) return null;

    // Build the list of gaps.  There are N+1 gaps for N non-float children.
    //
    // Gap layout:
    //   gap 0 (rawIndex=0): before first child, lineY = first child's top.
    //   gap i (rawIndex=i): between child[i-1] and child[i],
    //                        lineY = midpoint of child[i-1].bottom and child[i].top.
    //   gap N (rawIndex=N): after last child, lineY = last child's bottom.
    final n = nonFloats.length;
    final gapLineYs = <double>[];

    // Gap 0: before the first child.
    final firstPd = nonFloats.first.parentData as DocumentBlockParentData;
    gapLineYs.add(firstPd.offset.dy);

    // Gaps 1..N-1: between consecutive children.
    for (var i = 0; i < n - 1; i++) {
      final prevPd = nonFloats[i].parentData as DocumentBlockParentData;
      final nextPd = nonFloats[i + 1].parentData as DocumentBlockParentData;
      final prevBottom = prevPd.offset.dy + nonFloats[i].size.height;
      final nextTop = nextPd.offset.dy;
      gapLineYs.add((prevBottom + nextTop) / 2.0);
    }

    // Gap N: after the last child.
    final lastPd = nonFloats.last.parentData as DocumentBlockParentData;
    gapLineYs.add(lastPd.offset.dy + nonFloats.last.size.height);

    // Find the gap whose lineY is closest to localOffset.dy.
    var closestRawIndex = 0;
    var closestDist = (gapLineYs[0] - localOffset.dy).abs();
    for (var i = 1; i <= n; i++) {
      final dist = (gapLineYs[i] - localOffset.dy).abs();
      if (dist < closestDist) {
        closestDist = dist;
        closestRawIndex = i;
      }
    ...
    // Convert raw gap index to post-removal index.
    //
    // After removing the dragged node, all gaps with rawIndex > dragNodeIndex
    // shift down by 1.  Gaps rawIndex == dragNodeIndex and rawIndex ==
    // dragNodeIndex + 1 both correspond to the dragged node landing back in
    // its original position (index == dragNodeIndex).
    final int postRemovalIndex;
    if (closestRawIndex == dragNodeIndex || closestRawIndex == dragNodeIndex + 1) {
      postRemovalIndex = dragNodeIndex;
    } else if (closestRawIndex > dragNodeIndex) {
      postRemovalIndex = closestRawIndex - 1;
    } else {
      postRemovalIndex = closestRawIndex;
    }

    return BlockInsertionGap(index: postRemovalIndex, lineY: gapLineYs[closestRawIndex]);
  }
