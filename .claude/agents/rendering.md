...
3. Ask the `qa` agent to run the gate for the rendering layer — zero issues.
4. For visual changes: ask the `qa` agent to update goldens via `scripts/ci/flutter_test.sh --update-goldens test/src/rendering/` on Linux only (MCP has no `--update-goldens` flag).
...
Ask the `qa` agent to update goldens: `scripts/ci/flutter_test.sh --update-goldens test/src/rendering/` (script fallback — MCP has no `--update-goldens` flag)
