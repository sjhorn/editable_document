...
void main() {
  ...
  // ---------------------------------------------------------------------------
  // BlockInsertionGap / getInsertionGapAtOffset
  // ---------------------------------------------------------------------------

  group('RenderDocumentLayout — getInsertionGapAtOffset', () {
    // Helper that builds a 3-block layout with blockSpacing=0 and explicit
    // heights so that positions are deterministic.
    //
    // Layout (blockSpacing = 0):
    //   child 'a' — y: [0 .. 50)
    //   child 'b' — y: [50 .. 100)
    //   child 'c' — y: [100 .. 150)
    //
    // Gap midpoints:
    //   gap 0 (before 'a') lineY = 0
    //   gap 1 (between 'a'/'b') lineY = 50
    //   gap 2 (between 'b'/'c') lineY = 100
    //   gap 3 (after 'c') lineY = 150
    RenderDocumentLayout _threeBlockLayout() {
      final a = _imageBlock('a', requestedWidth: 400, requestedHeight: 50);
      final b = _imageBlock('b', requestedWidth: 400, requestedHeight: 50);
      final c = _imageBlock('c', requestedWidth: 400, requestedHeight: 50);
      return _layout(children: [a, b, c], blockSpacing: 0.0);
    }

    test('BlockInsertionGap equality', () {
      const g1 = BlockInsertionGap(index: 1, lineY: 50.0);
      const g2 = BlockInsertionGap(index: 1, lineY: 50.0);
      const g3 = BlockInsertionGap(index: 2, lineY: 100.0);
    ...
    test('BlockInsertionGap toString contains index and lineY', () {
      const g = BlockInsertionGap(index: 3, lineY: 75.0);
      expect(g.toString(), contains('3'));
      expect(g.toString(), contains('75'));
    });

    test('returns null when fewer than two non-float children', () {
      // Single child — no gap can be formed.
      final layout =
          _layout(children: [_imageBlock('only', requestedWidth: 400, requestedHeight: 50)]);
      final result = layout.getInsertionGapAtOffset(const Offset(200, 25), 'only');
      expect(result, isNull);
    });

    test('returns null when fewer than two non-float children (empty)', () {
      final layout = RenderDocumentLayout();
      layout.layout(const BoxConstraints(maxWidth: 400), parentUsesSize: true);
      final result = layout.getInsertionGapAtOffset(const Offset(0, 0), 'none');
      expect(result, isNull);
    });

    test('returns null when dragNodeId not found among children', () {
      final layout = _threeBlockLayout();
      final result = layout.getInsertionGapAtOffset(const Offset(200, 25), 'not_a_real_id');
      expect(result, isNull);
    });

    test('pointer above all blocks returns gap index 0', () {
      final layout = _threeBlockLayout();
      // Pointer is at y=-10, which is closest to gap 0 (lineY=0).
      final gap = layout.getInsertionGapAtOffset(const Offset(200, -10), 'b');
      expect(gap, isNotNull);
      // dragging 'b' (raw index 1) to raw gap 0 → post-removal index = 0
      expect(gap!.index, 0);
    });

    test('pointer between blocks a and b returns gap at index 1 (dragging c)', () {
      final layout = _threeBlockLayout();
      // Pointer near y=50, between 'a' and 'b'.
      // Dragging 'c' (raw index 2): raw gap 1 → post-removal index = 1
      final gap = layout.getInsertionGapAtOffset(const Offset(200, 50), 'c');
      expect(gap, isNotNull);
      expect(gap!.index, 1);
    });

    test('pointer between blocks b and c returns gap at index 2 (dragging a)', () {
      final layout = _threeBlockLayout();
      // Pointer near y=100, between 'b' and 'c'.
      // Dragging 'a' (raw index 0): raw gap 2 is after 'a', so post-removal index = 2 - 1 = 1
      final gap = layout.getInsertionGapAtOffset(const Offset(200, 100), 'a');
      expect(gap, isNotNull);
      // raw gap 2 > dragNodeIndex 0, so post-removal = 2 - 1 = 1
      expect(gap!.index, 1);
    });

    test('pointer below all blocks returns last gap (dragging a)', () {
      final layout = _threeBlockLayout();
      // Pointer at y=200, closest to gap 3 (lineY=150).
      // Dragging 'a' (raw index 0): raw gap 3 > dragNodeIndex 0 → post-removal = 3 - 1 = 2
      final gap = layout.getInsertionGapAtOffset(const Offset(200, 200), 'a');
      expect(gap, isNotNull);
      expect(gap!.index, 2);
    });

    test('post-removal index: dragging index-0 node to gap after index-2 returns index 2', () {
      final layout = _threeBlockLayout();
      // Pointer well below all blocks → raw gap 3 (lineY=150).
      // Dragging 'a' (raw index 0), raw gap 3 > 0 → post-removal = 3 - 1 = 2
      final gap = layout.getInsertionGapAtOffset(const Offset(200, 999), 'a');
      expect(gap, isNotNull);
      expect(gap!.index, 2);
    });

    test('dragging back to same gap returns dragNodeIndex', () {
      final layout = _threeBlockLayout();
      // Dragging 'b' (raw index 1) and pointer is at y=25 (closest to gap 0 or gap 1?).
      // gap 0 lineY=0, gap 1 lineY=50; y=25 is equidistant — either is acceptable.
      // But we want to test the "same position" case: drag 'b' to gap 1 (rawIndex=1,
      // which equals dragNodeIndex) → post-removal index = 1.
      // Use y=50 to hit gap 1 exactly.
      final gap = layout.getInsertionGapAtOffset(const Offset(200, 50), 'b');
      expect(gap, isNotNull);
      // raw gap 1 == dragNodeIndex 1 → post-removal = dragNodeIndex = 1
      expect(gap!.index, 1);
    });

    test('dragging to gap immediately after same node returns dragNodeIndex', () {
      final layout = _threeBlockLayout();
      // Gap 2 (lineY=100) is rawIndex=2 == dragNodeIndex+1 for 'b' (dragNodeIndex=1).
      // So gap 2 is "would land back at same position" → post-removal = dragNodeIndex = 1.
      final gap = layout.getInsertionGapAtOffset(const Offset(200, 100), 'b');
      expect(gap, isNotNull);
      expect(gap!.index, 1);
    });

    test('lineY is 0 for gap before first child', () {
      final layout = _threeBlockLayout();
      final gap = layout.getInsertionGapAtOffset(const Offset(200, -999), 'c');
      expect(gap, isNotNull);
      expect(gap!.lineY, 0.0);
    });

    test('lineY equals total height for gap after last child', () {
      final layout = _threeBlockLayout();
      final gap = layout.getInsertionGapAtOffset(const Offset(200, 999), 'a');
      expect(gap, isNotNull);
      expect(gap!.lineY, closeTo(150.0, 0.5));
    });

    test('skips float children when computing gaps', () {
      // Layout: 'a' (non-float), 'f' (float), 'b' (non-float).
      // Only 'a' and 'b' participate in gap computation → 3 gaps total.
      final a = _imageBlock('a', requestedWidth: 400, requestedHeight: 50);
      final f = _imageBlock(
        'f',
        requestedWidth: 100,
      ...
      final b = _imageBlock('b', requestedWidth: 400, requestedHeight: 50);
      final layout = _layout(children: [a, f, b], blockSpacing: 0.0);

      // With only 2 non-float children, 3 gaps exist (before a, between a/b, after b).
      // Dragging 'b' (non-float index 1): pointer above 'a' → raw gap 0 → post-removal index 0.
      final gap = layout.getInsertionGapAtOffset(const Offset(200, -10), 'b');
      expect(gap, isNotNull);
      expect(gap!.index, 0);
    });

    test('midpoint lineY between two blocks', () {
      // Two blocks of height 50 with no spacing:
      // block 'a' y:[0..50), block 'b' y:[50..100).
      // Gap 1 midpoint = (50 + 50) / 2 = 50 (top of 'b' == bottom of 'a').
      final a = _imageBlock('a', requestedWidth: 400, requestedHeight: 50);
      final b = _imageBlock('b', requestedWidth: 400, requestedHeight: 50);
      final layout = _layout(children: [a, b], blockSpacing: 0.0);

      // Probe at y=50 — should be closest to gap 1.
      final gap = layout.getInsertionGapAtOffset(const Offset(200, 50), 'a');
      expect(gap, isNotNull);
      expect(gap!.lineY, closeTo(50.0, 1.0));
    });

    test('midpoint lineY with blockSpacing > 0', () {
      // Two blocks of height 50 with spacing 20:
      // block 'a' y:[0..50), block 'b' y:[70..120).
      // Gap 1 midpoint = (50 + 70) / 2 = 60.
      final a = _imageBlock('a', requestedWidth: 400, requestedHeight: 50);
      final b = _imageBlock('b', requestedWidth: 400, requestedHeight: 50);
      final layout = _layout(children: [a, b], blockSpacing: 20.0);

      // Probe at y=60 — should be closest to gap 1.
      final gap = layout.getInsertionGapAtOffset(const Offset(200, 60), 'a');
      expect(gap, isNotNull);
      // Gap 1 lineY should be ~60 (midpoint of the 20px spacing gap).
      expect(gap!.lineY, closeTo(60.0, 1.0));
    });
  });
}
