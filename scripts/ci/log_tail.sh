...
log_file() {
  case "$1" in
    test)             echo "/tmp/ed_test_full.txt" ;;
    test_fail)        echo "/tmp/ed_test_fail.txt" ;;
    test_summary)     echo "/tmp/ed_test_summary.txt" ;;
    analyze)          echo "/tmp/ed_analyze_full.txt" ;;
    analyze_errors)   echo "/tmp/ed_analyze_errors.txt" ;;
    analyze_summary)  echo "/tmp/ed_analyze_summary.txt" ;;
    format)           echo "/tmp/ed_format_full.txt" ;;
    format_summary)   echo "/tmp/ed_format_summary.txt" ;;
    fix)              echo "/tmp/ed_fix_full.txt" ;;
    fix_summary)      echo "/tmp/ed_fix_summary.txt" ;;
    coverage)         echo "/tmp/ed_coverage_summary.txt" ;;
    gate)             echo "/tmp/ed_gate_summary.txt" ;;
    *)                echo "/tmp/ed_${1}_full.txt" ;;
  esac
}
...
case "$CMD" in
  summary)
    echo "=== editable_document CI log summaries ==="
    echo ""
